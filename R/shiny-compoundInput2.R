...
#' @param inputId character, shiny UI input id
#' @param label character, will show on each groups
#' @param components `HTML` tags generated by \code{htmltools} that defines and
#' combines UI within groups
...
#' specifing color of each group labels,
...
compoundInput2 <- function(
  inputId, label = 'Group', components = htmltools::tagList(),
  ...
  ...make_ui = function(ind){
    ...
    as.call(list(
      ...
      as.call(list(
        quote(htmltools::tags$fieldset),
        as.call(list(
          quote(htmltools::tags$legend),
          style = sprintf(
  ...
  comp_ui = quote(htmltools::div(
    id = inputId,
    class = 'dipsaus-compound-input',
    htmltools::singleton(htmltools::tags$head(
      htmltools::tags$link(rel="stylesheet", type="text/css", href="dipsaus/dipsaus.css"),
      htmltools::tags$script(src="dipsaus/dipsaus-compoundInput.js")
    )),
    htmltools::div(
...
#' @param value list of lists, see \code{compoundInput2} or exxamples
